...
public class Enemy extends Entity {
		...
		if(dir == 1) {
			if(World.isFree((int)(x + speed), (int)y)) {
				...
				if(World.tiles[xnext + ynext * World.WIDTH] instanceof WallTile && World.tiles[xnext + ynext * World.WIDTH].solid == false) {
					World.tiles[xnext + ynext * World.WIDTH] = new FloorTile(xnext * 16, ynext * 16, Tile.TILE_AR_DIA);
				}
				dir = -1;
				left = true;
				right = false;
	...
	public void render(Graphics g) {
		...
		g.fillRect((int)(x - Camera.x),(int)(y - 8 - Camera.y), 20, 7);
		g.setColor(Color.green);
		g.fillRect((int)(x - Camera.x),(int)(y - 8 - Camera.y), curLife, 7);
